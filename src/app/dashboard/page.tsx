...
export default function DashboardPage() {
  const { user, loading, checkin, canCheckin } = useUser();
    ...
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-white/40 text-sm capitalize">{user.faction} Faction</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-5 mb-8">
        <div className="md:col-span-2 bg-[#0d0d1a] border border-white/10 rounded-xl p-5">
          ...
          <XPBar currentXP={user.xp} level={user.level} xpForNextLevel={user.xpForNextLevel} />
        </div>
        <StreakCounter streak={user.streak.current} canCheckin={canCheckin} onCheckin={checkin} />
      </div>

      <div className="grid md:grid-cols-4 gap-4 mb-8">
        {[
          { label:'$MURPH Balance', value:user.murphBalance.toLocaleString(), color:'#00ff88', href:'/dashboard/wallet' },
          ...
          { label:'Longest Streak', value:`${user.streak.longest} days`, color:'#ff6600', href:'/dashboard' },
        ].map(s => (
